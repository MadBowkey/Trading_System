...
| M01-C03 | Modul 01 | Widerspruch in Audit-Klassifikation: bestehende Vorlagen mit `kritisch/mittel/neutral/entfernen` vs. neue Vorgabe `CRITICAL/HIGH/MEDIUM/LOW`. | HIGH | audit-übergreifend | offen | Muss vor konsistenter Priorisierung harmonisiert werden. |
| M01-C04 | Modul 01 | Universe-Konfiguration nur als `universe.example.yaml` explizit sichtbar; produktives Pendant nicht eindeutig festgelegt. | LOW | 01, 05 | offen | Risiko von Fehlkonfiguration bei Übergang in Betrieb. |
